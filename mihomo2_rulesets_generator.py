...
CONFIG = {
    ...
    'base_url': "https://cs2.ip.thc.org/",  # 规则源地址（需替换为实际可用源）
    'num_latest_files': 8,                 # 最新文件数量
    'num_splits': 5,                       # 每个文件分割份数（固定3组）
    ...
    'max_files_per_compile': 3,           # 单次编译最大文件数（减少以降低内存使用）
    ...
    'download_timeout': 900.0,             # 总下载超时（秒）
    ...
    'max_compile_attempts': 5,         # 编译最大重试次数
